...
### Hibernate 5.3 and GORM 7.x

Grails 4.x supports a minimum version of Hibernate 5.3 and GORM 7.x. Several changes have been made to GORM to support the newer version of Hibernate and simplify GORM itself.
...
The command will download an upgraded gradle profile. You will notice changes in the `/gradle/wrapper` folder as well
as the `gradlew` script files after the command completes. Note that Gradle 5.0 requires Java 8 or later to run.

If you are upgrading a multi-project build to Gradle 5.0, go to the the top level and run the `gradlew` upgrade command.
In order to upgrade the sub-projects, go into the `gradle-wrapper.properties` and adjust the `distributionUrl` property
to download the correct version of the gradlew-x.x-bin.zip, where `x.x` is the version number.
...
Note Hibernate requires a transaction for all operations, reads included. Grails Controllers do not have `@Transactional`
by default. Therefore if code in a Controller uses GORM without a Grails Service, it must be annotated with `@Transactional`.

### Micronaut HTTP Client

Grails 4 applications are recommended to upgrade to use the Micronaut HTTP Client. The Grails Datastore Rest Client is
no longer included in the default Grails 4 distribution.
...
If the client is used in the application code and not just in testing, change the dependency qualifier to `compile`.

Micronaut HTTP client is a feature rich client implementation that has both a low-level API and a higher level AOP-driven API.

Learn more at https://docs.micronaut.io/latest/guide/index.html#httpClient[Micronaut HTTP Client] and
https://guides.micronaut.io/micronaut-http-client-groovy/guide/index.html[Micronaut HTTP Client Guide]
